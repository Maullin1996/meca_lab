...
`meca_lab` is past the initial scaffold: `lib/main.dart` bootstraps `atomic_design` tokens, `SharedPreferences`, and a `ProviderScope`, then hands off to `lib/app.dart` (`App`), which gates on the initial auth-session check before mounting `MaterialApp.router`. State management is Riverpod 3+ with `riverpod_generator` (`@riverpod`/`@Riverpod(...)` codegen, no hand-written `NotifierProvider`, including family providers for per-id state like `deviceDetailControllerProvider(deviceId)`). Routing is `go_router`, with routes declared only for features that actually exist (`/login`, `/dashboard`, `/devices/:id`). Clean Architecture per feature (`domain/data/presentation`) is established and in active use — see "Cómo trabajar en este repo" below and the `meclab-flutter-dashboard-demo` skill for the full convention. `auth`, `dashboard`, and `device_detail` are complete (all three layers, tests passing); `alerts` and `setpoints` don't exist yet.
